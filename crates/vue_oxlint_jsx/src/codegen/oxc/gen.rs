...
impl Gen for Directive<'_> {
  fn r#gen(&self, p: &mut Codegen, _ctx: Context) {
    ...
    p.print_with_mapping(self.expression.span, |p| {
      quote.print(p);
...
impl Gen for SwitchStatement<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    p.print_ascii_byte(b')');
    p.print_curly_braces(self.cases.is_empty(), |p| {
...
impl Gen for SwitchCase<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    p.print_stmts(&self.consequent, ctx);
  }
}
...
impl Gen for Function<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    p.wrap(wrap, |p| {
      ...
      p.print_with_mapping(self.params.span, |p| {
        p.print_ascii_byte(b'(');
...
impl Gen for FunctionBody<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    let single_line = self.is_empty();
    p.print_curly_braces(single_line, |p| {
  ...
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    if let Some(specifiers) = &self.specifiers {
      if specifiers.is_empty() {
        p.print_str("{}");
        p.print_str("from");
        p.print_with_mapping(self.source.span, |p| {
          p.print_ascii_byte(b'"');
  ...
  fn r#gen(&self, p: &mut Codegen, _ctx: Context) {
    match &self.key {
      ImportAttributeKey::Identifier(identifier) => {
        identifier.print(p, Context::empty());
      }
      ImportAttributeKey::StringLiteral(literal) => {
        literal.print(p, Context::empty());
      }
    }
    p.print_colon();
    self.value.print(p, Context::empty());
  }
}
...
impl Gen for IdentifierReference<'_> {
  fn r#gen(&self, p: &mut Codegen, _ctx: Context) {
    ...
    p.print_space_before_identifier();
    p.print_with_mapping(self.span(), |p| {
      p.print_str(name);
    });
  }
}
  ...
  fn gen_expr(&self, p: &mut Codegen, precedence: Precedence, ctx: Context) {
    ...
    p.print_with_mapping(self.span(), |p| {
      if let Some(raw) = &self.raw {
        ...
        let wrap = precedence >= Precedence::Multiply
...
impl GenExpr for CallExpression<'_> {
  fn gen_expr(&self, p: &mut Codegen, precedence: Precedence, ctx: Context) {
    ...
    p.wrap(wrap, |p| {
      ...
      p.print_arguments(&self.arguments, ctx);
    });
  }
}
  ...
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    if let Expression::FunctionExpression(func) = &self.value {
      ...
      if self.method || is_accessor {
        ...
        p.print_with_mapping(func.params.span, |p| {
          p.print_ascii_byte(b'(');
...
impl Gen for AssignmentTargetPropertyIdentifier<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    if ident_name == self.binding.name.as_str() {
      ...
      // `({x: a} = y);`
      p.print_with_mapping(self.binding.span, |p| {
...
impl Gen for ClassBody<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    p.print_curly_braces(self.body.is_empty(), |p| {
...
impl Gen for JSXEmptyExpression {
  fn r#gen(&self, _p: &mut Codegen, _ctx: Context) {}
}
...
impl Gen for JSXExpression<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    if let Self::EmptyExpression(expr) = self {
      ...
      p.print_expression(self.to_expression());
    }
  }
}
...
impl Gen for JSXElement<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    p.print_with_mapping(self.opening_element.span, |p| {
      p.print_ascii_byte(b'<');
    ...
    if let Some(closing_element) = &self.closing_element {
      p.print_with_mapping(closing_element.span, |p| {
        p.print_str("</");
...
impl Gen for TSTypeLiteral<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    p.print_curly_braces(self.members.is_empty(), |p| {
...
impl Gen for TSIndexSignature<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    for (index, parameter) in self.parameters.iter().enumerate() {
      ...
      p.print_with_mapping(parameter.span, |p| {
...
impl Gen for TSModuleBlock<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    p.print_curly_braces(is_empty, |p| {
      p.print_directives_and_statements(&self.directives, &self.body, ctx);
    });
    p.needs_semicolon = false;
  }
...
impl Gen for TSInterfaceDeclaration<'_> {
  fn r#gen(&self, p: &mut Codegen, ctx: Context) {
    ...
    p.print_curly_braces(self.body.body.is_empty(), |p| {
