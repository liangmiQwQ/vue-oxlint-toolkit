...
/// Receives generated source ranges for AST nodes printed by [`Codegen`].
pub trait CodegenHook {
  fn record(&mut self, span: Span, virtual_start: u32, virtual_end: u32);
}

impl<F> CodegenHook for F
where
  F: FnMut(Span, u32, u32),
{
  fn record(&mut self, span: Span, virtual_start: u32, virtual_end: u32) {
    self(span, virtual_start, virtual_end);
  }
}
...
pub struct Codegen<'a> {
  ...
  node_hook: Option<Box<dyn CodegenHook + 'a>>,
}
...
impl<'a> Codegen<'a> {
  ...
  /// Set a callback invoked after each non-empty AST node is printed.
  #[must_use]
  pub fn with_codegen_hook(mut self, hook: impl CodegenHook + 'a) -> Self {
...
impl<'a> Codegen<'a> {
  ...
  #[inline]
  fn start_node(&self, span: Span) -> Option<u32> {
    if span.is_empty() || self.node_hook.is_none() {
      return None;
    }
    let Ok(start) = u32::try_from(self.code_len()) else {
      return None;
    };
    Some(start)
  }
  ...
  #[inline]
  fn finish_node(&mut self, span: Span, start: Option<u32>) {
    let Some(start) = start else {
      return;
    };
    let Ok(end) = u32::try_from(self.code.len()) else {
      return;
    };
    if let Some(hook) = self.node_hook.as_mut() {
      hook.record(span, start, end);
    }
  }
